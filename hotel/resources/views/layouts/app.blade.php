  ...
  <header class="border-b border-slate-200 bg-white/90 backdrop-blur">
    <div class="mx-auto flex h-20 max-w-7xl items-center justify-between px-6">
      <div class="flex items-center gap-10">
        <a href="{{ asset('/') }}" class="text-3xl font-black text-blue-700">{{ __('hotel.brand') }}</a>

        <nav class="hidden items-center gap-6 text-sm font-semibold text-slate-600 md:flex">
          <a class="transition hover:text-slate-900" href="{{ route('admin.dashboard') }}">{{ __('hotel.nav.explore') }}</a>
      ...
      <div class="flex items-center gap-4">
        ...
        <a href="#" class="text-sm font-semibold text-slate-600 transition hover:text-slate-900">{{ __('hotel.nav.contact') }}</a>
        <a href="{{ route('login') }}"
